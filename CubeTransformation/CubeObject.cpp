...
void InitCube()
{
	...
	GLfloat color[g_cube_vertex_num*3] = { 0.f, 0.f, 1.f 
							, 1.f, 0.f, 0.f
							, 0.f, 1.f, 0.f
							, 1.f, 0.f, 0.f
	...
	//// Position VBO
	// 꼭지점(vertex)의 좌표값을 저장하기 위한 VBO를 생성하고 자동으로 부여된 VBO의 이름을 g_two_tri_vbo_pos에 담는다.// Vertex Buffer Object
	...
	glGenBuffers(1, &g_cube_vbo_color);
	glBindBuffer(GL_ARRAY_BUFFER, g_cube_vbo_color);
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*3*g_cube_vertex_num, color, GL_STATIC_DRAW);	// color에 있는 내용을 g_two_tri_vbo_color로 옮긴다.


	//// Vertex Attribute
	// g_cube_vbo_pos를 위한 Vertex Attribute를 설정한다.
	glBindBuffer(GL_ARRAY_BUFFER, g_cube_vbo_pos);
	// 아래 두 함수의 첫 번째 파라메터 '0'은 g_two_tri_vbo_pos가 vshader.glsl와 fshader.glsl의 layout=0에 해당하는 변수에 연결 됨을 의미한다.
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0); 
	glEnableVertexAttribArray(0);

	// g_cube_vbo_color를 위한 Vertex Attribute를 설정한다.
	glBindBuffer(GL_ARRAY_BUFFER, g_cube_vbo_color);
