...
void DrawBasicShape(int i)
{
	...
	glDrawElements(GL_TRIANGLES, g_meshes[i]->num_faces()*3, GL_UNSIGNED_INT, 0);
}
...
void InitBasicShapeObjs()
{
	for ( int i=0; i<g_num_shapes; i++ )
	{
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*3*g_meshes[i]->num_vertices(), g_meshes[i]->points(), GL_STATIC_DRAW);


		// Normal VBO
		glGenBuffers(1, &g_vbo_normal[i]);
		glBindBuffer(GL_ARRAY_BUFFER, g_vbo_normal[i]);
		glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*3*g_meshes[i]->num_vertices(), g_meshes[i]->normals(), GL_STATIC_DRAW);
