...
void InitTreeModel()
{
	for ( int i=0; i<NUM_TREES; i++ )
	{
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*3*g_tree_mesh[i].num_vertices(), g_tree_mesh[i].points(), GL_STATIC_DRAW);

		// Color VBO
		glGenBuffers(1, &g_tree_vbo_color[i]);
		glBindBuffer(GL_ARRAY_BUFFER, g_tree_vbo_color[i]);
		glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*4*g_tree_mesh[i].num_vertices(), g_tree_mesh[i].colors(), GL_STATIC_DRAW);

		// Normal VBO
		glGenBuffers(1, &g_tree_vbo_normal[i]);
		glBindBuffer(GL_ARRAY_BUFFER, g_tree_vbo_normal[i]);
		glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*3*g_tree_mesh[i].num_vertices(), g_tree_mesh[i].normals(), GL_STATIC_DRAW);
